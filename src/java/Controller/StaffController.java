...
public class StaffController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (act.equals("view")) {
            StaffModel am = new StaffModel();
            ArrayList<User> list = am.getListStaff();
        ...
        if (act.equals("add")) {
            ...
            if (am.checkStaffExsist(id)) {
                ...
                if (am.addStaff(id, fname, add, phone, email, date, uname, pass, "4")) {
                    response.sendRedirect("view/Admin/staff.jsp");
                } else {
        ...
        if (act.equals("doUpdate")) {
            ...
            String date = request.getParameter("birthdaytime");
            StaffModel am = new StaffModel();
            if (am.updateStaff(id,fname, add,phone,email,date,uname,pass)) {
                response.sendRedirect("view/Admin/staff.jsp");
            } else {
                response.sendRedirect("error.jsp");
            }
